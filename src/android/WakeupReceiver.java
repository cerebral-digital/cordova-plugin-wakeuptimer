...
public class WakeupReceiver extends BroadcastReceiver {
	...
	private static String LOG_TAG = "WakeupReceiver";
	...
	public void onReceive(Context context, Intent intent) {
		...
		try {
			...
			if (extrasBundle!=null && extrasBundle.getString("type")!=null && extrasBundle.getString("type").equals("daylist")) {
				...
				Log.d(LOG_TAG,"resetting alarm at " + sdf.format(next) + " id " + extrasBundle.getInt("alarmId"));
	
				Intent reschedule = new Intent(context, WakeupReceiver.class);
				if (extras!=null) {
					reschedule.putExtra("extra", intent.getExtras().get("extra").toString());
				}
				reschedule.putExtra("alarmId", extrasBundle.getInt("alarmId"));
				reschedule.putExtra("day", WakeupPlugin.daysOfWeek.get(intent.getExtras().get("day")));
	
				PendingIntent sender = PendingIntent.getBroadcast(context, extrasBundle.getInt("alarmId") + WakeupPlugin.daysOfWeek.get(intent.getExtras().get("day")), intent, PendingIntent.FLAG_UPDATE_CURRENT);
				AlarmManager alarmManager = (AlarmManager) context.getSystemService(Context.ALARM_SERVICE);
				if (Build.VERSION.SDK_INT>=19) {
					alarmManager.setExactAndAllowWhileIdle(AlarmManager.RTC_WAKEUP, next.getTime(), sender);
				} else {
					alarmManager.setAndAllowWhileIdle(AlarmManager.RTC_WAKEUP, next.getTime(), sender);
				}
			}
